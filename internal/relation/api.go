...
func (r *Relation) syncConversationShowNameForFriend(ctx context.Context, friendUserID, remark string) {
	...
	if showName == "" {
		...
		if err != nil || friend == nil {
			log.ZWarn(ctx, "syncConversationShowNameForFriend failed", err, "friendUserID", friendUserID)
			return
		}
		showName = friend.ConversationShowName()
	}
	if showName == "" {
		log.ZInfo(ctx, "syncConversationShowNameForFriend failed", "friendUserID", friendUserID)
		return
	}
...
// syncPeerUserConversationDisplay refreshes single-chat show_name and message sender
// labels for a peer who is not in the local friend list (reverse one-way friendship).
func (r *Relation) syncPeerUserConversationDisplay(ctx context.Context, peerUserID string) error {
	log.ZInfo(ctx, "lintao syncPeerUserConversationDisplay start",
		"loginUserID", r.loginUserID,
		"peerUserID", peerUserID)
	if peerUserID == "" {
		log.ZWarn(ctx, "lintao syncPeerUserConversationDisplay skip empty peerUserID", nil,
			"loginUserID", r.loginUserID)
		return nil
	}
	friend, friendErr := r.db.GetFriendInfoByFriendUserID(ctx, peerUserID)
	if friendErr == nil && friend != nil {
		log.ZInfo(ctx, "lintao syncPeerUserConversationDisplay skip peer in local friend list",
			"loginUserID", r.loginUserID,
			"peerUserID", peerUserID,
			"localNickname", friend.Nickname,
			"localRemark", friend.Remark,
			"localFirstName", friend.FirstName,
			"localLastName", friend.LastName)
		return nil
	}
	log.ZInfo(ctx, "lintao syncPeerUserConversationDisplay peer not in local friend list, fetch user profile",
		"loginUserID", r.loginUserID,
		"peerUserID", peerUserID,
		"getFriendErr", friendErr)
	r.user.UserCache.Delete(peerUserID)
	peer, err := r.user.GetSingleUserFromServer(ctx, peerUserID)
	if err != nil {
		log.ZError(ctx, "lintao syncPeerUserConversationDisplay GetSingleUserFromServer failed", err,
			"loginUserID", r.loginUserID,
			"peerUserID", peerUserID)
		return err
	}
	showName := peer.DisplayName()
	if showName == "" {
		log.ZWarn(ctx, "lintao syncPeerUserConversationDisplay skip empty showName", nil,
			"loginUserID", r.loginUserID,
			"peerUserID", peerUserID,
	...
	log.ZInfo(ctx, "lintao syncPeerUserConversationDisplay apply profile update",
		"loginUserID", r.loginUserID,
		"peerUserID", peerUserID,
		"showName", showName,
	...
	log.ZInfo(ctx, "lintao syncPeerUserConversationDisplay done",
		"loginUserID", r.loginUserID,
		"peerUserID", peerUserID,
		"showName", showName)
	return nil
